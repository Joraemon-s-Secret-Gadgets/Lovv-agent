...
> 2026-07-02 보강: front `entry_type`, deterministic parser 우선, Bedrock structured extraction fallback, `gpt-oss-120b` baseline, Profile DynamoDB 연동 전제를 반영한다.
...
### 1.0 추출 파이프라인 ✅
Intent 파싱은 **1차 code parsing → 2차 Bedrock structured extraction** 순서로 실행한다.

1. **Deterministic code parser**
   - 구조 필드(`country`, `travel_month`, `travel_year`, `trip_type`, `active_required_themes`, `include_festivals`, `destination_id`, `user_location`)와 명시 키워드 기반 theme/region/preference를 먼저 정규화한다.
   - enum 필드는 코드에서 허용값을 검증한다. `congestion_pref = quiet | vibrant | neutral`, `transport_pref = walk | car | unknown`만 통과한다.
   - code parser가 확정하지 못한 값은 억지로 채우지 않고 기본값(`neutral`, `unknown`) 또는 `unsupported_conditions`/clarification flag로 남긴다.

2. **Bedrock structured extraction fallback**
   - code parser가 확정하지 못한 `soft_preference_query`, 긴 서사 압축, 암시적 혼잡 선호, 암시적 이동 선호만 보강한다.
   - baseline model은 Bedrock Runtime 기준 `openai.gpt-oss-120b-1:0`이다. repo 설정에서는 `LOVV_INTENT_LLM_MODEL_ID`가 우선하고, 없으면 공통 `LOVV_LLM_MODEL_ID` fallback을 쓴다.
   - LLM 출력은 반드시 `CitySelectInput`/Intent schema validation을 통과해야 한다. 실패하면 deterministic result로 degrade하고 provider 오류를 사용자 추천 흐름까지 전파하지 않는다.
   - LLM은 검색·스코어·도시 선택을 하지 않는다. 출력은 타입된 필드와 soft text까지만이다.

#### 1.0.1 `intent_output` 정본 예시 ✅
front 또는 intent bypass route가 이미 정규화한 `intent_output`을 보낸 경우에도 downstream 정본은 `city_select_input`으로 수렴한다.
...
```json
{
  "intent_output": {
    ...
    "cleaned_raw_query": "역사 유적과 자연을 함께 보고 싶고, 이동 부담은 적었으면 해요. 1박 2일이요.",
    "soft_preference_query": "고즈넉하고 차분하게 둘러볼 수 있는 분위기.",
...
`fixed_city_id`는 외부/legacy alias로만 허용하고, AgentV2 내부 정본은 `destination_id`와 `execution_mode`다.

### 1.1 구조 필드 정규화·검증 ✅
입력 필드: `entry_type · themes · tripType · travelMonth · destinationId · includeFestivals · country · userLocation · NL`.
...
| `entry_type` | front가 전달하는 생성/수정/확정 route 힌트 | route 계약은 `V2_11_MODIFY_IO_CONTRACT.md` 정본 |
| `NL` | 자유 텍스트 | §1.3에서 파싱 |
...
- **soft_query / soft_preference_query**: V1 prompt의 soft channel 의미를 유지한다. 분위기·감정 신호("조용", "생기 있게", "멍때릴")를 soft 검색·가중용 텍스트로 압축한다.
- **congestion_pref**: `quiet / vibrant / neutral`. 한적/북적 선호("사람 적은", "북적북적") → scoring의 congestion 가중 힌트.
...
→ `raw_query="바다"` · `theme_hint=[sea_coast]` · `soft_query="조용·위로·한적·따뜻"` · `congestion_pref=quiet` · 모순 없음 → 정상 생성.
**반례**: "그냥 요새 다 싫고 어디든 떠나고 싶어" → 명시 대상 0 → `underspecified`(테마 단정 안 함).

> 구현: deterministic parser가 먼저 명시 신호를 뽑고, 모호한 긴서사/soft channel만 **LLM 의미 압축**(Bedrock Converse)으로 보강한다. 검증은 judge로 **핵심 보존 + 과해석 0**(없는 테마 false-add = 0 목표). 긴서사·모호·모순은 동시 발생 가능하며 위 3단계가 순서대로 거른다.
...
- ⚠ **Bedrock fallback 운영 gate**: `LOVV_INTENT_LLM_MODEL_ID=openai.gpt-oss-120b-1:0` 설정, model access, schema retry, deterministic degrade 경로를 배포 전 검증해야 한다.
