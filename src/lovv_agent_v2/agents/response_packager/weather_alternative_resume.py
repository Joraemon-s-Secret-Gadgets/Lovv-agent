...
WEATHER_ALTERNATIVE_FAILURE_NOTICE = "실내 대체 후보가 부족해 현재 일정으로 유지합니다."
    ...
    return {
        "response": {
            ...
            "response_payload": package_recommendation_response(
                ...
                selected_city=_selected_city(state, output),
                recommendation_id=_recommendation_id(response),
                response_status="modification_pending",
            ),
...
def _alternative_itinerary(
    ...
    targets = tuple(
        ...
        if item_exposure(item) in WEATHER_SENSITIVE_EXPOSURES
    )
    if not targets:
        return tuple(dict(item) for item in itinerary)
    replacements = _replacement_candidates(state, planner_output, targets)
...
def _request_payload(state: Mapping[str, Any]) -> dict[str, Any]:
...
def _selected_city(state: Mapping[str, Any], planner_output: Mapping[str, Any]) -> Mapping[str, Any] | None:
...
def _planner_output(state: Mapping[str, Any]) -> Mapping[str, Any]:
...
def _recommendation_id(response: Mapping[str, Any]) -> str | None:
