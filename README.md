...
## LLM 사용 구성 (V1) — 어떤 모델 · 어느 위치에 · 왜

### 어떤 모델

| 구분 | 값 | 비고 |
| --- | --- | --- |
| 추론 LLM | `openai.gpt-oss-120b-1:0` | Amazon Bedrock에서 제공하는 OpenAI **gpt-oss 120B 오픈웨이트** 모델. **Bedrock Converse API**(`bedrock-converse` adapter)로 호출 |
| 임베딩 | `amazon.titan-embed-text-v2:0` | S3 Vectors 관광지 검색용 쿼리 임베딩 |
| 추론 강도 | `reasoning_effort="low"` (전 LLM 호출 공통) | gpt-oss는 추론을 완전히 끌 수 없어 `low`가 하한 — 지연·토큰을 줄이려 하한으로 고정 (`adapters/bedrock_converse.py`가 Converse `additionalModelRequestFields`로 전달) |

> 패키지 자체는 **모델 비종속** 설계입니다(`LlmSettings.model_id` 기본 미설정). 실제 모델 ID는 배포가 주입하며, **V1 배포(`agentcore/agentcore.json`)는 전 노드를 위 단일 모델로 설정**합니다.

### 어느 위치에 (LLM 호출 지점)

LLM은 파이프라인 전체가 아니라 **아래 3개 지점에서만** 호출됩니다. 도시 선택·일정 배치·단계 라우팅 같은 **핵심 결정은 전부 결정론(규칙 기반)** 입니다.

| 노드 / 위치 | LLM 역할 | 코드 |
| --- | --- | --- |
| **Intent** | 자연어 쿼리에서 안전한 신호(분위기 등) 추출 → structured output | `agents/intent.py` |
| **Candidate Evidence** | 선택 도시·장소의 한국어 추천 **근거 문구** 생성 | `agents/candidate_evidence.py` |
| **Planner (copy)** | 일정 설명·추천 이유 **텍스트** 생성 | `tools/explanation_composer.py` |

- **Supervisor**: 노드별 모델 ID 환경변수(`LOVV_SUPERVISOR_LLM_MODEL_ID`)는 예약돼 있으나, V1 라우팅은 **deterministic**으로 LLM을 호출하지 않습니다.
- **Festival Verifier**: 축제 날짜 검증은 deterministic으로 LLM을 호출하지 않습니다.
- 임베딩(titan)은 Intent가 만든 쿼리(raw/soft)를 Candidate Evidence의 S3 Vectors 검색 입력으로 변환할 때 사용됩니다.

노드별로 모델·adapter를 분리 지정할 수 있습니다: `LOVV_INTENT_LLM_MODEL_ID`, `LOVV_CANDIDATE_EVIDENCE_LLM_MODEL_ID`, `LOVV_PLANNER_LLM_MODEL_ID`, `LOVV_SUPERVISOR_LLM_MODEL_ID`(및 각 `_ADAPTER_ID`). 미설정 시 공통값 `LOVV_LLM_MODEL_ID`로 폴백합니다.

### 호출별 입력 · 출력

세 호출 모두 Converse **structured output(JSON 스키마 강제)** 으로 동작하며, 입력은 **공개-안전 요약**(내부 점수·벡터 거리·S3 URI·키 제외)만 전달합니다.

**① Intent** — `agents/intent.py`

- **입력:** `naturalLanguageQuery` + 구조화 컨텍스트(`themes` · `tripType` · `country` · `travelMonth` · `destinationId` 등). System prompt: `prompts/intent_normalization.v1.md`.
- **출력 (6필드 JSON):** `needs_clarification`, `clarifying_question`, `cleaned_raw_query`, `soft_preference_query`(분위기 묘사 HyDE 문장, 없으면 빈 문자열), `unsupported_conditions`, `handoff_notes`.
- 테마 매핑·실행 모드·`candidate_evidence_input` 등 **코어 필드는 LLM이 아니라 코드가 결정론으로 조립**합니다(LLM의 구조 필드는 폐기).

**② Candidate Evidence — 추천 근거 문구** — `agents/candidate_evidence.py`

- **입력:** 공개-안전 요약(선택 도시, 후보 장소 목록, 테마·쿼리 요약). 내부 점수·벡터 거리·S3 URI 제외.
- **출력:** `candidate_reason_claims[]` = `{ claim_id, scope, text_ko(한국어 근거 문구), evidence_refs, required_place_ids, public_eligible }`. 내부 토큰(`place_score` · `raw_s3_uri` · vector distance 등)이 섞이면 **출력 거부**(환각·내부정보 누출 방지).

**③ Planner copy/explanation** — `tools/explanation_composer.py`

- **입력:** 공개-안전 요약 — `selected_city`, query 요약, `final_itinerary_items`(배치된 일정 항목), 공개 가능한 `candidate_reason_claims`, `verified_festivals`, `validation_result`.
- **출력:** `{ item_copies[] = {item_ref, title, body, reason}, recommendation_reasons[], itinerary_flow_reason }` — 일정 항목별 **설명(body)·추천 이유(reason)** + 전체 추천 이유 목록 + 일정 흐름 설명.

### 왜 이렇게 사용했나

- **결정론 우선 + LLM 최소화:** 추천 품질·재현성·안전성을 위해 도시 선택/일정/라우팅은 규칙 기반으로 두고, LLM은 (1) 자연어 신호 추출, (2) 근거 문구, (3) 일정 설명 — **사람이 읽는 텍스트 생성**에만 제한합니다. LLM이 검색 결과나 일정을 임의로 바꾸지 못하게 하기 위함입니다.
- **Bedrock Converse 네이티브:** 외부 API 키 없이 AWS 자격증명 체인으로 호출하고, structured output(스키마 강제)·토큰/지연 관측을 X-Ray/CloudWatch 한 경로로 통합합니다.
- **gpt-oss-120b 선택 이유:** Bedrock에서 제공되는 오픈웨이트 모델로 한국어 생성과 structured output을 지원하면서, `reasoning_effort`로 추론 비용을 통제할 수 있습니다. 테스트상 검색·DB 지연은 1초 미만이고 전체 응답시간은 LLM이 지배하므로, 추론 강도를 **하한(low)** 으로 두는 것이 합리적입니다.
- **노드별 모델 분리 설계:** 향후 노드 특성에 맞춰 다른 모델로 교체·실험할 수 있도록 환경변수로 분리해 두었고, V1은 운영 단순화를 위해 단일 모델로 통일했습니다.

> 위 "왜"는 **코드·배포 설정에서 드러나는 엔지니어링 근거**입니다. (다른 후보 모델과의 정량 비교 같은 별도 선정 실험은 본 저장소에 포함돼 있지 않습니다.)
